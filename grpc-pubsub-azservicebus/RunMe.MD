...
2. Dapr provides capabilities (via its Building Blocks and Components) to services using it that are far beyond the capabilities offered by Service Meshes.  For example, Dapr provides capabilities like State Management, Pub/Sub, Resource Bindings and Triggers, Virtual Actors, and more.  These capabilities are broad "runtime" capabilities that also include facilitating services collaborating with each other via service-to-service invocation of service operations.  Please see this link for Dapr capabilities https://docs.dapr.io/concepts/overview/. 
