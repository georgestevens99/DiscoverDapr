...
2. **Deploy repo files** to required places outside the repo as follows:
    ...
    * If you are **NOT using Docker**, i.e. you installed Dapr with dapr init --slim, then do the following 2 items that manipulate yaml files so as to prevent dapr from generating fatal errors when it encounters components that require containers running in Docker and also to prevent your display being cluttered with non-fatal errors when dapr tries to write to the tracing port.  It you **ARE using Docker** *do NOT do the following two items* that manipulate yaml files.
      * With the exception of svs-pubsub-asbtopic.yaml, remove all other yaml files from your machine's DaprRuntimePathPrefix\\.dapr\components directory.  I do this by making another directory called .dapr\componentsOffline.  Then move all items in .dapr\components directory to .dapr\componentsOffline except leaving only svs-pubsub-asbtopic.yaml in .dapr\components directory.  This prevents fatal errors when dapr tries to use a component that depends on a container running in Docker.  For example when using the default pubsub.yaml component, or the default statestore.yaml component, or other components depending upon containers running in Docker.
      * In DaprRuntimePathPrefix\\.dapr\config.yaml change samplingRate: "1" to samplingRate: "0".  This prevents the display from being cluttered with non-fatal errors when dapr tries to write tracing data every second or so.
