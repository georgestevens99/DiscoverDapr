...
2. **Deploy repo files** to required places outside the repo as follows:
    ...
    * If you are **NOT using Docker**, i.e. you installed Dapr with dapr init --slim, then do the following so as to prevent dapr from fatal generating errors when it encounters components that require containers running in Docker and also to prevent your display being cluttered with non-fatal errors when dapr tries to write to the tracing port.
      * Remove all other yaml files from your machines DaprRuntimePathPrefix\\.dapr\components directory.  I do this by making another directory, .dapr\componentsOffline.  Then move all item in .dapr\components directory to .dapr\componentsOffline except leaving only svs-pubsub-asbtopic.yaml in .dapr\components directory.  This prevents fatal errors when dapr tries to use a component that depends on a container running in Docker.
      * Change samplingRate: "1" to samplingRate: "0" in DaprRuntimePathPrefix\\.dapr\config.yaml.  This prevents the display from being cluttered with non-fatal errors when dapr tries to write tracing data every second or so.
